	...
	public View getView(final int position, View convertView, ViewGroup parent) 
	{
	    ...
	    switch(type)
    	{
    		case ReportItem.ITEM_TYPE_PRONAME:
	             ...
	             view.setBackgroundColor(color.ivory);
	    ...
	    switch(type)
    	{
    		case ReportItem.ITEM_TYPE_PRONAME:
    			String s=mData.get(position).getMessage();
    			 ...
    			 //holder1.tv.setBackgroundColor(color.aqua);
    			 break; 
    		case ReportItem.ITEM_TYPE_PROPARR:
    			String s1 = null;
    			if(position==projectnum)
    			{
    				s1=mData.get(position).getData().get("place"); 
    			...
    			{
    				s1=mData.get(position).getData().get("carnum");
    				holder2.tvpara.addTextChangedListener(new TextWatcher() {     
    	                @Override  
    	                public void afterTextChanged(Editable s) {  
    	                    //将editText中改变的值设置的HashMap中  
    			...
    			{
    				s1=mData.get(position).getData().get("depart");
    				holder2.tvpara.addTextChangedListener(new TextWatcher() {     
    	                @Override  
    	                public void afterTextChanged(Editable s) {  
    	                    //将editText中改变的值设置的HashMap中  
